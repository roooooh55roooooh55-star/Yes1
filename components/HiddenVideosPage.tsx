...
const HiddenVideosPage: React.FC<HiddenVideosPageProps> = ({ interactions, allVideos, onRestore, onPlayShort, onPlayLong }) => {
  ...
  return (
    <div className="flex flex-col gap-8 pb-32 animate-in fade-in duration-500">
      <div className="p-6 rounded-3xl bg-red-600/10 border border-red-600/30">
        <h1 className="text-2xl font-black italic text-red-600">الفيديوهات المستبعدة</h1>
        ...
        {hiddenVideos.map((video) => (
          <div key={video.id || video.video_url} className="flex flex-col gap-2 relative">
            ...
            <button 
              ...
              className="absolute -top-2 -right-2 bg-green-600 text-white p-1.5 rounded-full shadow-lg border border-white/20 active:scale-90 transition-transform"
              title="استعادة الفيديو"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"/></svg>
            </button>
            <p className="text-[10px] font-bold line-clamp-1 text-center">{video.title}</p>
          </div>
        ))}
      </div>
    </div>
