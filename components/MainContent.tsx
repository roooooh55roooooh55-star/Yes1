      ...
      {displayVideos.map((video, i) => (
        ...
        >
          <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-xl mb-2 aspect-video bg-black">
            <video src={video.video_url} muted loop playsInline autoPlay className="w-full h-full object-cover opacity-80 pointer-events-none" />
...
const MainContent: React.FC<MainContentProps> = ({ videos, interactions, onPlayShort, onPlayLong, onViewUnwatched, loading }) => {
  // 1. فيديوهات "مكمل رعب" (فريدة تماماً)
  ...
  const excludedIds = useMemo(() => {
    const set = new Set();
    unwatchedVideos.forEach(v => set.add(v.id || v.video_url));
    interactions.dislikedIds.forEach(id => set.add(id));
    return set;
  }, [unwatchedVideos, interactions.dislikedIds]);

  // 2. نظام الإحلال التلقائي: نأخذ أول 4 فيديوهات ليست في قائمة المستبعدات
  const availableShorts = useMemo(() => 
    videos.filter(v => v.type === 'short' && !excludedIds.has(v.id || v.video_url)).slice(0, 4), 
    [videos, excludedIds]
  );
  
  const availableLongs = useMemo(() => 
    videos.filter(v => v.type === 'long' && !excludedIds.has(v.id || v.video_url)).slice(0, 4), 
    [videos, excludedIds]
  );
          ...
          {availableLongs.map((video) => (
             <div key={video.id || video.video_url} onClick={() => onPlayLong(video, true)} className="bg-neutral-900/40 rounded-[2.5rem] border border-white/5 overflow-hidden active:scale-[0.98] transition-all shadow-2xl">
              <div className="relative aspect-video w-full bg-black">
                <video src={video.video_url} muted autoPlay loop playsInline className="w-full h-full object-cover" />
